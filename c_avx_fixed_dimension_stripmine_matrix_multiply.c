...
typedef struct
{
    ...
    SAL_cf32 *left_matrix_rows[CMAT_MULX_FIXED_DIMENSION_STRIPMINE_MAX_NR_C];
    SAL_cf32 *right_matrix_rows[CMAT_MULX_FIXED_DIMENSION_STRIPMINE_MAX_DOT_PRODUCT_LENGTH];
    SAL_cf32 *output_matrix_rows[CMAT_MULX_FIXED_DIMENSION_STRIPMINE_MAX_NR_C];
    cmat_mulx_fixed_dimension_stripmine_func cmat_mulx_func;
} matrix_context;
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    {
        mexErrMsgIdAndTxt ("c_avx_fixed_dimension_accumulate_matrix_multiply:c", "Inputs are not complex single 2D arrays");
    }
    ...
    if ((context.nr_c <= CMAT_MULX_FIXED_DIMENSION_STRIPMINE_MAX_NR_C) &&
        (context.dot_product_length <= CMAT_MULX_FIXED_DIMENSION_STRIPMINE_MAX_DOT_PRODUCT_LENGTH))
    {
        context.cmat_mulx_func = cmat_mulx_fixed_dimension_stripmine_functions[context.nr_c][context.dot_product_length];
    }
