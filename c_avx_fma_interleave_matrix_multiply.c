...
typedef struct
{
    ...
    SAL_cf32 *left_matrix_rows[NR_C_MAX];
    SAL_cf32 *right_matrix_rows[NR_C_MAX];
    SAL_cf32 *output_matrix_rows[NR_C_MAX];
...
static void timed_c_matrix_multiply (void *arg)
{
    ...
    context->rc = cmat_mulx_avx_fma_dot_product_length_8 (context->left_matrix_rows[0], context->left_matrix.tcols,
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (nlhs != 2)
    {
        mexErrMsgIdAndTxt ("c_avx_fma_interleave_matrix_multiply:a", "Incorrect number of outputs");
    }
    if (nrhs != 5)
    {
        mexErrMsgIdAndTxt ("c_avx_fma_interleave_matrix_multiply:b", "Incorrect number of inputs");
    }
    ...
    {
        mexErrMsgIdAndTxt ("c_avx_fma_interleave_matrix_multiply:c", "Inputs are not complex single 2D arrays");
    }
    ...
    {
        mexErrMsgIdAndTxt ("c_avx_fma_interleave_matrix_multiply:d", "Inconsistent matrix dimensions");
    }
    ...
    if ((context.nr_c <= NR_C_MAX) && (context.dot_product_length == 8))
    {
        ...
        {
            mexErrMsgIdAndTxt ("c_avx_fma_interleave_matrix_multiply:e", "cmat_mulx failed with rc=%u", context.rc);
        }
