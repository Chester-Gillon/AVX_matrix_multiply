function mat_mulx_attribs = generate_avx_fixed_dimension_accumulate_matrix_multiply (mat_mulx_attribs)
    [~,~] = mkdir ('mat_mul_fragments/cmat_mulx_avx_accumulate');
...
''};
    ...
    for nr_c = 2:20
        for dot_product_length = 2:20
            ...
            func_filename = sprintf ('mat_mul_fragments/cmat_mulx_avx_accumulate/%s.c', func_name);
            ...
            if nargin >= 1
                mat_mulx_attribs{func_name, 'multiply_type'} = {'cmat_mulx_avx_accumulate'};
    ...
    for right_row = 0:dot_product_length-1
        ...
        for output_row = 0:nr_c-1
            ...
            elseif right_row < dot_product_length-1
                ...
                fprintf(c_fid, '        _mm256_storeu_ps (&(C[%u] + c_c)->real, _mm256_add_ps (output_r%u, _mm256_addsub_ps (_mm256_mul_ps (right_r_i, left_r%u_c%u_r),\n',output_row,output_row,output_row,right_row);
                fprintf(c_fid, '                              %s                                       %s                    _mm256_mul_ps (right_i_r, left_r%u_c%u_i))));\n',output_row_padding,output_row_padding,output_row,right_row);
                num_vfops = num_vfops + 4;
            end
        end
